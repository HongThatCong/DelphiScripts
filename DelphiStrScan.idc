...
{
    ASSERT((UNICODE_STRING <= st) && (st <= PASCAL_STRING), sprintf("Script bug - Unknown string type %d\n", st));
    ...
    MAP_NAME(st, PASCAL_STRING, "PascalString");
...
{
    auto len, easz, oldstrtype, ret;

    oldstrtype = get_inf_attr(INF_STRTYPE);
    ...
    set_inf_attr(INF_STRTYPE, oldstrtype);
...
// Only scan 7bit and printable ASCII chars
static isPascalString(addr)
{
    auto len, ia, ch;

    len = get_wide_byte(addr++);
    if (0xFF == len || len < 3) // Min len is 3
        return 0;

    for (ia = addr; ia < addr + len; ia++)
    {
        ch = get_wide_byte(ia);
        if (ch < 0x20 || ch > 0x7E)
            return 0;
...
static createPascalString(addr)
{
    auto len;

    len = get_wide_byte(addr);
    del_items(addr, DELIT_SIMPLE, len + 1);
    create_strlit(addr, len + 1);

    DBG_MSG_2("0x%X: %s\n", addr, getStringType(PASCAL_STRING));
    return 1;
}

static scanPascalStrings()
{
    auto start_ea, end_ea, ea, oldStrType, nfixed;

    start_ea = get_inf_attr(INF_MIN_EA);
    end_ea = get_inf_attr(INF_MAX_EA);

    oldStrType = get_inf_attr(INF_STRTYPE);
    set_inf_attr(INF_STRTYPE, ASCSTR_PASCAL);
    ...
    while (ea < end_ea)
    {
        ...
        else if (isPascalString(ea))
        {
            createPascalString(ea);
            ea = ea + get_wide_byte(ea) + 1;
            nfixed++;
        }
        else
        {
...
static main()
{
    auto oldstatus, ea, start_ea, end_ea, nfixed;
    ...
    nfixed = scanPascalStrings();

     // Scan for Delphi strings
    ea = find_binary(start_ea, SEARCH_DOWN, "FF FF FF FF");
